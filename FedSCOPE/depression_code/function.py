...
# 6. 公平性指标计算（修改为使用 Gender 作为敏感变量，Male 为特权）
def compute_fairness_metrics(y_true, y_pred, sensitive_features):
    privileged_group = (sensitive_features == A_PRIVILEGED)  # Male
    unprivileged_group = (sensitive_features == A_UNPRIVILEGED)  # Female
    TPR_privileged = (
...
def test_inference_modified(global_model, test_loader, model_class):
    global_model.eval()
    y_true, y_pred, y_prob, gender_values = [], [], [], []
    loss_total = 0.0
    criteria = nn.CrossEntropyLoss()
    with torch.no_grad():
        for batch in test_loader:
            if len(batch) == 3:
                X_batch, y_batch, gender_batch = batch
            else:
                X_batch, y_batch = batch
                gender_batch = torch.tensor([], dtype=torch.long).to(HYPERPARAMETERS['DEVICE'])
            X_batch, y_batch, gender_batch = (
                X_batch.to(HYPERPARAMETERS['DEVICE']),
                y_batch.to(HYPERPARAMETERS['DEVICE']),
                gender_batch.to(HYPERPARAMETERS['DEVICE'])
            )
    ...
    for i in range(len(y_true)):
        gender = gender_values[i]
        depression = y_true[i]
        prediction = y_pred[i]
        if gender == 1 and depression == 1:
            per_category['(G+,D+)']['total'] += 1
            if prediction == 1:
                per_category['(G+,D+)']['correct'] += 1
        elif gender == 1 and depression == 0:
            per_category['(G+,D-)']['total'] += 1
            if prediction == 0:
                per_category['(G+,D-)']['correct'] += 1
        elif gender == 0 and depression == 1:
            per_category['(G-,D+)']['total'] += 1
            if prediction == 1:
                per_category['(G-,D+)']['correct'] += 1
        elif gender == 0 and depression == 0:
            per_category['(G-,D-)']['total'] += 1
            if prediction == 0:
                per_category['(G-,D-)']['correct'] += 1
    assert len(y_true) == len(gender_values), "y_true 和 gender_values 的长度不匹配！"
    return loss_test, accuracy_score(y_true, y_pred), f1, fairness_metrics, per_category
...
def assign_sample_weights_to_clients(clients_data, weights, sensitive_column, class_column='Depression'):
    for client_id, client_data in clients_data.items():
        s = client_data['y'].cpu().numpy()
        c = client_data['y'].cpu().numpy()  # 此处按目标变量计算权重
        # 此处简单遍历 (由于数据已处理，直接取1.0权重即可)
...
def compute_personalization_index(clients, global_model, model_class):
    ...
    for client in clients:
        # 全局模型评估（客户端验证集准确率）
        global_model.eval()
        _, _, global_acc = client.evaluate()  # 此处 evaluate 返回 (spd, eod, local_acc)
        # 个性化微调：复制全局模型，在本地训练数据上训练一轮（不聚合）
