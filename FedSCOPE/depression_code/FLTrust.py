...
class Server:
    ...
    def _average_weights(self, client_updates):
        ...
        for key in client_updates[next(iter(client_updates))].keys():
            aggregated_weights[key] = torch.stack([client_updates[client_id][key].float().to(DEVICE)
                                                    for client_id in client_updates], dim=0).mean(dim=0)
    ...
    def run_round(self, global_model, test_df, y_test_values, model_class):
        """
        运行一轮全局训练，根据算法不同调用不同的聚合方法，并收集各客户端当前模型状态用于后续计算 PP。
        """
        client_deltas = {}
        local_losses = []
        if self.algorithm in ['FLTrust', 'FLTrust_RW']:
            ...
            if self.server_data is not None:
                ...
                server_X_tensor = torch.tensor(self.server_data.drop('Depression', axis=1).values, dtype=torch.float32).to(HYPERPARAMETERS['DEVICE'])
                server_y_tensor = torch.tensor(self.server_data['Depression'].values, dtype=torch.long).to(HYPERPARAMETERS['DEVICE'])
